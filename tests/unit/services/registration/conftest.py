...
from src.models import Account, Invite
from tests.fakes import fake_data_for_invite_row, fake_email_schemas
    ...
    async def _check_row() -> None:
        async with async_session_maker() as session:
            row_existed = await session.execute(
                select(Invite).filter_by(**fake_data_for_invite_row),
            )
            if row_existed:
